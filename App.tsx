...
const BackgroundDecorator = () => {
  ...
  return (
    <div className="fixed inset-0 pointer-events-none overflow-hidden -z-10">
      ...
      <div 
        className="absolute w-[800px] h-[800px] rounded-full blur-[140px] opacity-20 dark:opacity-40 transition-opacity duration-1000"
        style={{
          left: mousePos.x - 400,
          top: mousePos.y - 400,
          background: 'radial-gradient(circle, rgba(73, 136, 196, 0.3) 0%, transparent 70%)'
        }}
      />
      ...
      <div className="floating-particle w-[250px] h-[250px] md:w-[400px] md:h-[400px] bg-brand-accent/20 bottom-[-5%] right-[-5%]" style={{ animationDelay: '-8s' }} />
    </div>
  );
};
  ...
  return (
    <div className="min-h-screen relative text-brand-deep overflow-x-hidden transition-colors duration-500">
      ...
      <main className="relative z-10">
        {/* HERO SECTION */}
        <section id="home" className="pt-28 md:pt-48 lg:pt-64 pb-16 md:pb-32 lg:pb-48 px-4 sm:px-6 lg:px-8 flex items-center min-h-[90vh]">
          <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-12 lg:gap-24 items-center w-full relative">
            <div className="space-y-6 md:space-y-8 order-2 md:order-1 text-center md:text-left">
              <div className="flex items-center justify-between md:justify-start gap-3 w-full animate-intro-right" style={{ animationDelay: '200ms' }}>
                {/* Social Links Bar */}
                <div className="flex items-center gap-3">
                  ...
                  <a href={SOCIAL_LINKS.whatsapp} target="_blank" rel="noopener noreferrer" className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl border border-brand-medium/10 flex items-center justify-center transition-all hover:scale-110 hover:bg-brand-deep hover:text-white dark:hover:bg-white dark:hover:text-[#1C4D8D] bg-white dark:bg-slate-800 text-brand-medium shadow-sm"><MessageCircle size={18} /></a>
                  <a href="mailto:mfredebel@gmail.com" className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl border border-brand-medium/10 flex items-center justify-center transition-all hover:scale-110 hover:bg-brand-deep hover:text-white dark:hover:bg-white dark:hover:text-[#1C4D8D] bg-white dark:bg-slate-800 text-brand-medium shadow-sm"><Mail size={18} /></a>
                </div>
                <AvailabilityBadge className="hidden md:flex absolute right-0" />
              </div>

              {/* Main Headline with Typewriter */}
              <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-black tracking-tight leading-[1.1] text-brand-deep animate-intro-up" style={{ animationDelay: '400ms' }}>
                Hello, I'm <br />
                <span className="gradient-text typewriter-cursor inline-block min-h-[1.2em]">{typewriterText}</span>
              </h1>
              <p className="text-lg md:text-2xl max-w-lg mx-auto md:mx-0 leading-relaxed text-brand-medium font-semibold animate-intro-up" style={{ animationDelay: '600ms' }}>Building high-performance digital solutions and automated systems. Specializing in scalable, design-driven architecture.</p>
              
              {/* Primary Call-to-Actions */}
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 md:gap-4 pt-4 animate-intro-up" style={{ animationDelay: '800ms' }}>
              ...
              <div className="flex justify-center md:justify-start pt-2 animate-intro-up" style={{ animationDelay: '1000ms' }}><LocationBadge /></div>
            </div>

            {/* Profile Imagery with Floating animations */}
            <div className="relative order-1 md:order-2 flex flex-col items-center lg:items-end animate-intro-scale" style={{ animationDelay: '300ms' }}>
              ...
              <AvailabilityBadge className="md:hidden mt-6 animate-intro-up" style={{ animationDelay: '1200ms' }} />
            </div>
          </div>
        </section>

        {/* INTEGRATED ABOUT & SKILLS */}
        <section id="about" className="py-20 md:py-32 px-4 bg-brand-deep/[0.06] border-y border-brand-deep/5 relative overflow-hidden">
          {/* Animated background Blobs */}
          <div className="blob-container">
            <div className="blob w-64 h-64 top-1/4 -left-20" />
            <div className="blob w-80 h-80 bottom-1/4 -right-20" style={{ animationDelay: '-4s' }} />
            <div className="blob w-48 h-48 top-1/2 left-1/2 opacity-[0.05]" style={{ animationDelay: '-7s' }} />
          </div>
                  ...
                  <div key={category} className="p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] border border-brand-medium/10 bg-white dark:bg-slate-900 shadow-xl shadow-brand-medium/5 transition-all hover:-translate-y-1">
                    ...
                    <div className="space-y-4 md:space-y-5">
                      {SKILLS.filter(s => s.category === category).map((skill, idx) => (
                        <div key={skill.name} className="flex items-center gap-3 md:gap-4 skill-animate group/item" style={{ animationDelay: `${(catIdx * 0.1) + (idx * 0.05)}s` }}>
                ...
                <div key={project.title} className="group relative bg-white dark:bg-slate-900 rounded-[2rem] overflow-hidden border border-brand-medium/10 shadow-xl transition-all hover:-translate-y-2 hover:shadow-2xl">
                  ...
                  <div className="aspect-[16/10] overflow-hidden relative">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-brand-deep/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-end p-8">
                      <a href={project.link} className="w-12 h-12 bg-white rounded-full flex items-center justify-center text-brand-deep transform translate-y-4 group-hover:translate-y-0 transition-all duration-500 delay-100"><ExternalLink size={20} /></a>
                    </div>
                  </div>
                  {/* Project Metadata */}
            ...
            <div className="rounded-[3rem] md:rounded-[5rem] p-6 sm:p-10 md:p-20 relative overflow-hidden glass-card noise-bg contact-bg-flow min-h-[600px] flex items-center">
              ...
              <div className="absolute inset-0 pointer-events-none overflow-hidden">
                ...
                {glitters.map((glitter) => (
                  <div key={glitter.id} className="glitter" style={{ left: glitter.left, top: glitter.top, width: glitter.size, height: glitter.size, animationDuration: glitter.duration, animationDelay: glitter.delay }} />
                ))}
              </div>
